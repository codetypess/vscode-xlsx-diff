...
import { InlineCellInput } from "../shared/inline-cell-input";
import {
    areMeasuredItemWidthsEqual,
    classNames,
    getVsCodeApi,
    isTextInputTarget,
    measureElementPixelWidth as measureSharedElementPixelWidth,
    measureMaximumDigitWidthFromDocument as readMaximumDigitWidthFromDocument,
    measureSheetTabWidths as readSheetTabWidths,
} from "../shared/webview-helpers";
import {
...
import { getFreezePaneCountsForCell, hasLockedView } from "../shared/view-lock";
...
export function EditorBootstrapApp() {
    const vscode = getVsCodeApi<EditorWebviewOutgoingMessage>();
    ...
    onMount(() => {
        const measureMaximumDigitWidthFromDocument = () => {
            setMaximumDigitWidth(readMaximumDigitWidthFromDocument(DEFAULT_MAXIMUM_DIGIT_WIDTH_PX));
        };

        const measureSheetTabsViewport = () => {
            setSheetTabsViewportWidth(measureSharedElementPixelWidth(sheetTabsViewportElement));
        };

        const measureSheetTabWidths = () => {
            const nextMeasuredTabWidths = readSheetTabWidths(sheetTabsMeasureElement, {
                itemSelector: '[data-role="sheet-tab-measure"]',
                maxWidth: SHEET_TAB_VISIBLE_MAX_WIDTH,
            });

            setMeasuredSheetTabWidths((current) =>
                areMeasuredItemWidthsEqual(current, nextMeasuredTabWidths)
                    ? current
