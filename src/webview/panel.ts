...
const STRINGS: Strings =
    ((globalThis as Record<string, unknown>).__XLSX_DIFF_STRINGS__ as Strings) ?? DEFAULT_STRINGS;
...
function syncCellDisplay(
    ...
    const value = pendingEdit
        ? pendingEdit.value
        : getCellModelValue(rowNumber, columnNumber, side);
...
function applyPendingEditStyles(): void {
    ...
    for (const pane of document.querySelectorAll<HTMLElement>(".pane__table[data-side]")) {
        ...
        for (const colHeader of pane.querySelectorAll<HTMLElement>(
...
document.addEventListener("click", (event: MouseEvent) => {
    ...
    switch (action) {
        case "set-filter":
            vscode.postMessage({
                type: "setFilter",
                filter: target.getAttribute("data-filter") as RowFilterMode,
            });
            return;
        case "set-sheet":
            vscode.postMessage({
