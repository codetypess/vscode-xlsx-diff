...
function clampPage(sheet: SheetSnapshot, currentPage: number): number {
	const totalPages = Math.max(
		1,
		Math.ceil(Math.max(sheet.rowCount, 1) / DEFAULT_PAGE_SIZE),
	);
...
function clampSelectedCell(
	...
	return {
		...
		columnNumber: Math.min(
			Math.max(selectedCell.columnNumber, 1),
			sheet.columnCount,
		),
	};
}
...
function createWorkbookFileView(workbook: WorkbookSnapshot): WorkbookFileView {
	return {
		...
		modifiedTimeLabel:
			workbook.modifiedTimeLabel ?? formatModifiedTime(workbook.modifiedTime),
...
function getWorkbookSummary(workbook: WorkbookSnapshot): {
	...
	return workbook.sheets.reduce(
		(summary, sheet) => ({
			...
			totalNonEmptyCells:
				summary.totalNonEmptyCells + Object.keys(sheet.cells).length,
		}),
...
export function normalizeEditorPanelState(
	...
	const activeSheetEntry =
		sheetEntries.find((entry) => entry.key === state.activeSheetKey) ??
		sheetEntries[0] ??
		null;
...
export function moveEditorPageCursor(
	...
	const adjacentSheet =
		direction < 0
			? sheetEntries[activeSheetIndex - 1]
			: sheetEntries[activeSheetIndex + 1];
	...
	const targetPage =
		direction < 0
			? clampPage(adjacentSheet.sheet, Number.MAX_SAFE_INTEGER)
			: 1;
...
export function createEditorRenderModel(
	...
	const columns = Array.from(
		...
		(_, index) => getColumnLabel(index + 1),
	);
	...
	const sheetIndex = sheetEntries.findIndex(
		(entry) => entry.key === activeSheetEntry.key,
	);
