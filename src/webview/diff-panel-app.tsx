...
function getSourceRowNumber(row: DiffPanelRowView | null, side: Side): number | null {
    if (!row) {
        return null;
    }

    return side === "left" ? row.leftRowNumber : row.rightRowNumber;
}

function getDefaultSideForRow(row: DiffPanelRowView | null): Side {
...
function getDefaultSelectionForFilter(
    ...
    if (filter === "diffs") {
        ...
        const firstDiffRowNumber = sheet.diffRows[0];
        if (!firstDiffRowNumber) {
            return null;
        }
    ...
    if (filter === "same") {
        ...
        return createSelection(
            ...
            firstSameRowNumber,
            1,
            getDefaultSideForRow(runtime?.rowByNumber.get(firstSameRowNumber) ?? null)
        );
    }
        ...
        <div className="diff-paneHeader">
            ...
            <div ref={viewportRef} className="diff-columnsViewport">
                <div
                    ...
                    style={{
                        width: columnWindow.totalWidth,
                        transform: `translateX(-${scrollLeft}px)`,
                    }}
                >
                    {columnWindow.leadingSpacerWidth > 0 ? (
                        <div
                            className="diff-columnSpacer"
                            style={{ width: columnWindow.leadingSpacerWidth }}
                            aria-hidden="true"
                        />
                    ) : null}
                    ...
                    {columnWindow.trailingSpacerWidth > 0 ? (
                        <div
                            className="diff-columnSpacer"
                            style={{ width: columnWindow.trailingSpacerWidth }}
                            aria-hidden="true"
                        />
                    ) : null}
                </div>
...
function App(): React.JSX.Element {
    ...
    const ensureColumnVisible = React.useEffectEvent((columnNumber: number) => {
        ...
        if (cellRight > viewportRight) {
            syncHorizontalScroll(cellRight - columnWindow.viewportWidth);
        }
    });
    ...
    React.useEffect(() => {
        ...
        ensureColumnVisible(selectedCell.columnNumber);
    }, [
        activeSheetViewKey,
        columnViewportWidth,
    ...
    const updateSelectedCell = React.useEffectEvent((nextSelection: CellSelection | null) => {
        ...
        if (nextSelection) {
            ensureColumnVisible(nextSelection.columnNumber);
        }
