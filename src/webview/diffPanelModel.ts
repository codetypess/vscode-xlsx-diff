...
function applyLeftCell(
    ...
    const current = ensureCell(rowsByNumber, cell.rowNumber, cell.columnNumber);
    current.key = cell.key;
    current.address = cell.address;
...
function createSheetView(sheet: SheetDiffModel): DiffPanelSheetView {
    ...
    for (const cell of Object.values(sheet.leftSheet?.cells ?? {})) {
        applyLeftCell(rowsByNumber, cell);
    }

    for (const cell of Object.values(sheet.rightSheet?.cells ?? {})) {
        applyRightCell(rowsByNumber, cell);
    }
    ...
    const rows: DiffPanelRowView[] = [...rowsByNumber.entries()]
        .sort(([leftRowNumber], [rightRowNumber]) => leftRowNumber - rightRowNumber)
        .map(([rowNumber, rowCells]) => {
            let diffTone: CellDiffStatus = diffRowsSet.has(rowNumber)
            ...
            return {
                rowNumber,
                hasDiff: diffRowsSet.has(rowNumber),
                diffTone,
                cells,
            };
        });
