...
function areCellsEqual(
	...
	return (
		leftCell.displayValue === rightCell.displayValue &&
...
function createSheetDiff(
	...
	if (!leftSheet || !rightSheet) {
		...
		const keys = new Set([
			...Object.keys(leftSheet.cells),
			...Object.keys(rightSheet.cells),
		]);
...
export function buildWorkbookDiff(
	...
	const rightByName = new Map(
		rightWorkbook.sheets.map((sheet) => [sheet.name, sheet] as const),
	);
	...
	return {
		...
		totalDiffRows: diffSheets.reduce(
			(total, sheet) => total + sheet.diffRows.length,
			0,
		),
		totalDiffCells: diffSheets.reduce(
			(total, sheet) => total + sheet.diffCellCount,
			0,
		),
	};
}
