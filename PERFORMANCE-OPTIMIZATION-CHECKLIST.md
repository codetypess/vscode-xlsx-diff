...
- [x] 展示 `defined names` 差异（已接入工作簿结构 diff 判定与面板提示）
- [x] 展示 `freeze pane` 差异（已接入结构性 diff 判定与面板提示）
- [x] 展示 sheet 隐藏/显示状态变化（已接入结构性 diff 判定与面板提示）
- [x] 展示 sheet 顺序变化（已接入结构性 diff 判定与面板提示）
- [x] 展示工作表增删改名的更明确提示（已在结构提示中区分新增、删除、重命名）
...
- 已新增真实 `.xlsx` fixture 生成脚本、案例目录与 7 条回归样本
